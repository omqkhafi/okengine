/**
 * Schema field rows — typed badge + optional pill per field.
 */

import type { JSX } from "react";
import { SheetGrid } from "@/components/ui/sheet-form.tsx";
import { SchemaColumnMarks } from "@/features/store/schema/schema-constraint-icon.tsx";
import { cn } from "@/lib/utils";
import { fieldConstraintHint, type FormField } from "../lib/fields-from-schema.ts";
import { schemaTypeBadgeClass } from "../lib/schema-type-visual.ts";

/** Props for {@link SchemaFields}. */
export interface SchemaFieldsProps {
  readonly fields: readonly FormField[];
  readonly emptyLabel?: string;
  /** `grid` — 4-col dock (Parameters). Default is a stacked list. */
  readonly layout?: "list" | "grid";
}

/**
 * Human-readable schema field list.
 *
 * @param props - Fields from {@link fieldsFromSchema}
 */
export function SchemaFields({
  fields,
  emptyLabel = "No schema fields declared.",
  layout = "list",
}: SchemaFieldsProps): JSX.Element {
  if (fields.length === 0) {
    return (
      <p className="text-xs text-muted-foreground" data-slot="schema-fields-empty">
        {emptyLabel}
      </p>
    );
  }
  if (layout === "grid") {
    return (
      <div data-slot="schema-fields" data-layout="grid">
        <SheetGrid columns={3}>
          {fields.map((f) => (
            <SchemaFieldRow key={f.path} field={f} depth={0} cell />
          ))}
        </SheetGrid>
      </div>
    );
  }
  return (
    <ul className="flex flex-col" data-slot="schema-fields">
      {fields.map((f) => (
        <SchemaFieldRow key={f.path} field={f} depth={0} />
      ))}
    </ul>
  );
}

function SchemaFieldRow({
  field,
  depth,
  cell = false,
}: {
  readonly field: FormField;
  readonly depth: number;
  readonly cell?: boolean;
}): JSX.Element {
  const isEnum = field.type === "enum" && field.enumValues;
  const constraint = fieldConstraintHint(field);
  const Tag = cell ? "div" : "li";
  return (
    <Tag className={cell ? "min-w-0 border-b border-border/50" : undefined}>
      <div
        className={
          cell
            ? "flex items-center gap-1.5 px-2 py-1.5"
            : "flex items-center gap-2 py-1"
        }
        style={cell ? undefined : { paddingLeft: depth * 14 }}
        data-slot="schema-field-row"
      >
        <span
          className="shrink-0"
          data-slot="schema-field-keys"
          title={constraintTitle(field)}
        >
          <SchemaColumnMarks
            primaryKey={field.primaryKey}
            foreignKey={field.foreignKey}
            unique={field.unique}
            empty="none"
          />
        </span>
        <span className="min-w-0 truncate font-mono text-[11px] font-medium text-foreground/90">
          {field.name}
        </span>
        {!field.required ? (
          <span
            className="shrink-0 rounded border border-border/60 bg-muted/30 px-1 py-px text-[9px] font-medium tracking-wide text-muted-foreground uppercase"
            data-slot="schema-field-optional"
          >
            optional
          </span>
        ) : null}
        {field.pii ? (
          <span
            className="shrink-0 rounded border border-amber-500/40 bg-amber-500/10 px-1 py-px text-[9px] font-medium tracking-wide text-amber-800 uppercase dark:text-amber-300"
            data-slot="schema-field-pii"
          >
            pii
          </span>
        ) : null}
        {field.sensitive ? (
          <span
            className="shrink-0 rounded border border-rose-500/40 bg-rose-500/10 px-1 py-px text-[9px] font-medium tracking-wide text-rose-700 uppercase dark:text-rose-300"
            data-slot="schema-field-sensitive"
          >
            sensitive
          </span>
        ) : null}
        {constraint && !isEnum ? (
          <span
            className="shrink-0 rounded border border-blue-500/35 bg-blue-500/10 px-1 py-px font-mono text-[9px] font-medium tracking-wide text-blue-800 dark:text-blue-300"
            data-slot="schema-field-constraint"
            title={`Allowed range ${constraint}`}
          >
            {constraint}
          </span>
        ) : null}
        <span
          className={cn(
            "ml-auto shrink-0 rounded border px-1.5 py-px font-mono text-[9px] font-medium tracking-wide",
            schemaTypeBadgeClass(field.type),
          )}
          data-slot="schema-field-type"
          title={isEnum ? field.enumValues?.join(" | ") : field.type}
        >
          {field.type}
        </span>
      </div>
      {field.children?.map((c) => (
        <SchemaFieldRow key={c.path} field={c} depth={depth + 1} cell={cell} />
      ))}
    </Tag>
  );
}

function constraintTitle(field: FormField): string {
  const bits: string[] = [];
  if (field.primaryKey) bits.push("Primary key");
  if (field.foreignKey) {
    const target = field.references
      ? [field.references.table, field.references.column].filter(Boolean).join(".")
      : null;
    bits.push(target ? `Foreign key → ${target}` : "Foreign key");
  }
  if (field.unique) bits.push("Unique");
  return bits.length > 0 ? bits.join(" · ") : "None";
}
