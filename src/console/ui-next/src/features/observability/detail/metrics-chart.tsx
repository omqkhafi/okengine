...
import type { ObservabilityChartRow } from "../lib/time-buckets.ts";

/** Series ink — sky volume, rose errors, teal latency. */
export const OBSERVABILITY_SERIES = {
...
export interface MetricsChartProps {
  readonly data: readonly ObservabilityChartRow[];
}
...
export function MetricsChart({ data }: MetricsChartProps): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <SeriesBar
        dataKey={OBSERVABILITY_SERIES.requests.key}
        fill={OBSERVABILITY_SERIES.requests.color}
        radius={2}
      />
      <Line
        dataKey={OBSERVABILITY_SERIES.errors.key}
        showMarkers={false}
        stroke={OBSERVABILITY_SERIES.errors.color}
        strokeWidth={2}
      />
      <Area
        dataKey={OBSERVABILITY_SERIES.p95.key}
        fadeEdges
        fill={OBSERVABILITY_SERIES.p95.color}
        fillOpacity={0.28}
        showMarkers={false}
        strokeWidth={2}
        yAxisId="right"
...
function tooltipRows(point: Record<string, unknown>): TooltipRow[] {
  ...
  return [
    ...
    {
      color: OBSERVABILITY_SERIES.errors.color,
      label: OBSERVABILITY_SERIES.errors.label,
      value: errors,
    },
    {
      color: OBSERVABILITY_SERIES.p95.color,
      label: OBSERVABILITY_SERIES.p95.label,
