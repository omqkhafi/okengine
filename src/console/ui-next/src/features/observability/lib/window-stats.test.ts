...
import { parseObservabilityWindow, percentile, windowStatsForRuns } from "./window-stats.ts";

describe("parseObservabilityWindow", () => {
  test("keeps known tokens and falls back", () => {
    expect(parseObservabilityWindow("7d")).toBe("7d");
    expect(parseObservabilityWindow("nope")).toBe("1h");
    expect(parseObservabilityWindow(undefined)).toBe("1h");
  });
});
