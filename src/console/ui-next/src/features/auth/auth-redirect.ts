 ...
 *
 * Only `/flows`, `/units`, and `/store` (plus their search) are legal.
...
const SHELL_PATHS = new Set(["/flows", "/units", "/store"]);
...
/**
 * Keep only an in-console shell href (`/flows` | `/units` | `/store` + search).
 *
