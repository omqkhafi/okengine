...
/** Forbidden external vault driver id, split so this file does not match itself. */
const FORBIDDEN_EXT_VAULT = ["open", "bao"].join("");
/** Forbidden vault-provider alias, split so this file does not match itself. */
const FORBIDDEN_VAULT_ALIAS = ["hashi", "corp"].join("");

/** Lockfiles regenerated by `bun install` — not source residue. */
const LOCKFILE_PREFIXES = [
  "bun.lock:",
  "packages/create-oke/templates/blank/bun.lock:",
  "packages/create-oke/templates/shorter/bun.lock:",
];
...
describe("removed vault driver gate", () => {
  ...
  test("tracked tree has zero matches for the removed external vault driver", () => {
    // Community-plugin notes in changelog / site docs may mention the name.
    assertZeroGitGrep(FORBIDDEN_EXT_VAULT, ["changelog.md:", "site/content/docs/"]);
  });

  test("tracked tree has zero matches for the removed vault alias outside docs notes", () => {
    assertZeroGitGrep(FORBIDDEN_VAULT_ALIAS, ["changelog.md:", "site/content/docs/"]);
  });
});
