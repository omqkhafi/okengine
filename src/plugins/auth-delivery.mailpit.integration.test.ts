 ...
 *
 * Skips visibly (warn + return) when the Docker daemon is unavailable —
 * same convention as `src/docker/stack.integration.test.ts`.
 */
...
describe("auth delivery — Mailpit integration", () => {
  test("magic-link request lands a real email containing the token and link", async () => {
  ...
  test("email-otp request lands a real email containing the OTP", async () => {
    if (!(await dockerAvailable())) {
      console.warn("skipping: docker daemon not available");
      return;
    }
