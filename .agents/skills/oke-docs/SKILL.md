...
| Components       | Fumadocs: `Cards`/`Card`, `Callout`, `Steps`/`Step`, `Accordions`/`Accordion`, `Tabs`/`Tab`, `TypeTable`. Docs visuals: `Features`, `CollapseDiagram`, `ManifestPipeline`, `Surfaces`, `Vocabulary`, `FlowShape`, `DevModes`, `ClientLoop`, `DriftBoard`, `CollapseBoard`. Element visuals: `FlowTriggers`, `SignalDelivery`, `StoreFacets`, `StoreFacetMark`, `StoreKvTtl`, `StoreFilesVariants`, `StoreIndexModes`, `ClockSchedules`, `GatePipeline`, `VaultResolution`, `ChannelPhysics`, `AiGuardrails`. |
