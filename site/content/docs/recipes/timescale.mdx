...
<Callout title="Keep timescaledb first">
  Overwriting `command` with only `pg_stat_statements` breaks hypertables. This recipe
  preloads `timescaledb,pg_stat_statements`. Then `CREATE EXTENSION pg_stat_statements`.
</Callout>
