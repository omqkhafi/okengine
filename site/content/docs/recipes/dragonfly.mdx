...
Dragonfly is the default `store.kv` image in `dev` and `prod` — a multi-threaded,
memory-efficient reimplementation of the Redis protocol. Same `redis://` wire format,
same `redis` driver, zero Flow changes. Pin Redis or Valkey when you want those engines
instead.
