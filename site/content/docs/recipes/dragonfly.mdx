...
`store.kv.durable` mounts `/data` and sets `--dbfilename dump` plus an env-gated
`--snapshot_cron` (minute granularity when a durable store is declared).

Dragonfly has **no AOF**. Writes between snapshots are volatile. Pin Redis or Valkey on
`images.store.kvDurable` for AOF-equivalent RPO.
