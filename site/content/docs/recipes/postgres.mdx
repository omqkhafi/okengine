...
<Callout title="Preload, then create">
  `pg_stat_statements` must load at postmaster start. `CREATE EXTENSION` alone is not
  enough. Recreate `store-sql` after a recipe change — the data volume can stay.
</Callout>
