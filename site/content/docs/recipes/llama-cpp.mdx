...
Balanced / Smart → curated Docker Hub `ai/` pick) / `--yes` for `smollm2`.
...
`oke ai setup` tiers (Ultra Fast → `smollm2`) for smoke tests. Do not mount
random GGUF files from the internet.
