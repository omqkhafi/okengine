...
create-oke **AI setup → Recommended** picks OpenRouter with `openrouter/free`
(no Docker) and prompts for `OPENROUTER_API_KEY`. `oke ai setup --provider openrouter`
writes the same `openai-compatible` driver wiring. Or create a key at
[openrouter.ai](https://openrouter.ai) and set `OPENROUTER_API_KEY`.
...
- [Models](/docs/elements/ai/models) — verified providers and limited compatibility
- [Local AI](/docs/recipes/local-ai) — local `/v1`
...
<Cards>
  ...
  <Card
    title="Local AI"
    description="llama.cpp / vLLM / SGLang."
