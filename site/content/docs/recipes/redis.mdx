...
`store.kv.durable` always mounts `store-kv-durable-data:/data`. AOF stays off
(`OKE_STORE_KV_DURABLE_APPENDONLY=no`) until the app declares `{ durable: true }`.

Then `--appendonly yes --appendfsync everysec` (~1s RPO). Recreate `store-kv-durable` after
that flip — the named volume stays. AOF is server-wide, not per-key.
