...
<Callout title="Console Performance">
  Store → KV band → **Performance** reads `INFO` / `SLOWLOG` on this instance.
  Those commands are server-wide, not scoped to `oke:kv:{ns}:`.
  `memory` (tests) returns `KvStatsUnsupported`.
</Callout>
