---
title: Docker
description: Deploy with plain docker compose — single production-grade docker-compose.yml, shared drivers, PgDog, readiness, graceful SIGTERM, and opt-in Caddy, Traefik, or nginx when you need an edge without an orchestrator.
icon: Package
source: README.md
---
...
<Callout>
  The monorepo `examples/keel` app is the work-management Compose fixture — `bun run dev:keel`, then
  `oke db seed` (Harbor GA story + ~500 tasks). Wipe with `bun run dev:keel:reset` (stops leftover
  ports, fresh `.env.local`).
</Callout>
...
When both `store.sql` and `pgdog` are pinned (templates' `dev`/`prod` default),
...
| **Traefik** (`traefik:v3.7`)    | `docker compose up --scale app=N`         | Docker labels on `app`; replicas auto-discovered and load-balanced |
...
`create-oke` asks **Add a reverse proxy…?** (or `--proxy caddy|traefik|nginx|none`).
...
- [Reverse proxy](/docs/deployment/reverse-proxy) — Caddy / Traefik / nginx in full
