...
<Callout>
  The monorepo `examples/keel` app is the work-management Compose fixture —
  `bun run dev:keel`, then `oke db seed` (Harbor GA story + ~500 tasks).
  Wipe with `bun run dev:keel:reset` (stops leftover ports, fresh `.env.local`).
</Callout>
