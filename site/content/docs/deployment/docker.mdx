...
<Callout>
  ...
  Wipe with `bun run dev:keel:reset` (stops leftover `:6530` / `:6533` / `:6535`
  listeners, fresh `.env.local`; next boot regenerates keys).
</Callout>
...
| **nginx** (`nginx:1.27-alpine`) | Static HTTP edge; TLS elsewhere           | Generated `nginx.conf` → `proxy_pass` to `app:6530` on `:80`       |
