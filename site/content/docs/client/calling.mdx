...
<Tabs items={["Minimal", "Narrow", "Failures", "Transport"]}>
...
create-oke starters ship `web/`: Vite owns `index.html` and proxies your Flow paths plus `/_oke`
to the app. Leave `PUBLIC_API_URL` unset in local web so `createClient("")` stays same-origin.
