...
```typescript
import { isOk } from "okengine/client";
import { matchError } from "okengine/client/explain";
...
Named arms narrow `data`. `_` is required — store auto-map, transport, and unknown domain codes
land there. `explain(error)` is the same collapse without a matcher (`kind`, `fields`, `retryable`).
...
`error.code === "FlightFull"` still narrows `error.data`. Use `explain` when you do not know the
code. HTTP status is on the response, not on `error`.
...
| `explain` / `matchError`                  | function  | `okengine/client/explain` — `kind`, `fields`, required `_` |
...
<Accordion title='error.code is "TransportError"'>
  ...
  `error.data.message` holds the text; `explain` maps HTTP 404 transport to kind `missing`.
</Accordion>
