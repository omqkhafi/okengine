...
| `retry.retries` | `number`                                       | `0`                | Extra attempts after the first. With `retry` set, a call retries when it is GET/QUERY or it carries an idempotency key. |
...
| `idempotencyKey` | `string` \| `false`                           | 21-character key   | `Idempotency-Key` on this call. `false` sends no header. Omitted on GET. |

With `retry` configured, a call retries when it is GET/QUERY or it carries an
idempotency key (automatic on every non-GET call). `idempotencyKey: false` runs
once unless that call passes `{ retry: true }`.

A `409` whose code is `IdempotencyInProgress` waits for `Retry-After` and retries
within those attempts. Other error envelopes are returned as-is.
...
A replay sets `meta.idempotentReplayed` when the server sends `Idempotent-Replayed: true`.
...
- [Routing](/docs/understand/routing) — `$routes` stamps
