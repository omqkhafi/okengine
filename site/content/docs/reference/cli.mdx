...
1. Prompts one-by-one for any Vault boot gaps (writes `.env.local`) — e.g. Keel's
   `OPENROUTER_API_KEY` when OpenRouter is the default AI.
2. Asks to run `oke db seed` when a seed module exists and `.oke/state.json` has
   not recorded that seed identity yet. Live key **`s`** still runs seed later.
