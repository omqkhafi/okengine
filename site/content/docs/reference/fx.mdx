...
<Callout title="The one rule">
  All world access goes through `fx`. No `node:` I/O, no raw `fetch` for side effects, no
  `Date.now()` — clocks, stores, channels, vault, and AI only through this object.
</Callout>
...
<Accordion title="OKE1001–1007 undeclared effect">
  An explicit `effects` block drifted from what `do` touches. Add the missing
  ledger entry, or drop the hand-written block so inference covers the Flow —
