...
Read-only flows cache automatically from inferred or ledgered `reads` — no `fx.cache`
call and no `cache:` default on the flow. Writes invalidate those keys. Use
`cache: false` to opt out, or `cache: "30s"` for a TTL. `fx.cache` is the manual
(tier-3) surface:
