...
<Accordion title="OKE1001–1007 / 1008 / 1009 undeclared effect">
  An explicit `effects` block drifted from what `do` touches. Add the missing ledger
  entry (`reads` · `writes` · `emits` · `sends` · `asks` · `embeds` · `secrets` ·
  `calls` · `fetches`), or drop the block so inference covers the Flow ([Errors](/docs/reference/errors)).
</Accordion>
