...
<Callout title="Extracted helpers">
  Type a helper's `fx` parameter as `Fx` (`import type { Fx } from "okengine"`).
  A narrower structural type will not match `store()` overloads. See
...
| `fx.auth.createApiKey({ name, scopes, expiresIn? })` | `write` `auth:api-keys` | Secret once. Creator is live `userId` / `scopes`. Session only. |
...
| `fx.auth.updateApiKey(id, …)` | `write` `auth:api-keys` | Name / scopes / expiry. Re-attenuates |
