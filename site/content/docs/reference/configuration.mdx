...
<Callout title="The one rule">
  Pin drivers and images in `oke.config.ts` for `dev` · `test` · `prod`. We test on what we
  deploy: Docker-first local stack, PGLite for tests, same protocol ids in production.
</Callout>
...
For `ai`, see [Recipes · AI](/docs/recipes#ai-local--self-hosted). For `proxy`, see [Caddy](/docs/recipes/caddy), [Traefik](/docs/recipes/traefik), or [nginx](/docs/recipes/nginx).
