...
- **Session and framing:** `frame-ancestors 'none'`, `SameSite=Strict`, step-up authentication before destructive actions. An expired operator session redirects to Sign in with `?next=` set to the current `/overview`, `/flows`, `/store`, or `/vault` href (search included); after login the operator returns there. Legacy `/units` rewrites to `/flows`. Any other `next` is dropped so the gate cannot be used as an open redirect.
