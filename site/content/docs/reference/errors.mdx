...
| `1018` | MCP tool duplicate   | Two MCP tool bindings share the same tool name    | Give each MCP tool exposure a unique name            |
| `1019` | undeclared fetch     | Flow fetches a host not in `effects.fetches`      | Add the hostname to the flow's `effects.fetches`     |
| `1020` | undeclared embed     | Flow embeds with a model not in `effects.embeds`  | Add it to the flow's `effects.embeds`                |
