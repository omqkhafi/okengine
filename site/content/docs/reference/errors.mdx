...
`DatabaseError` status: `not_null` / `check` → **422**, `retryable` → **503**, else **500**.
...
| Signal | Result |
| ------ | ------ |
| Postgres `23505` / `23P01` · SQLite UNIQUE | `Conflict` **409** |
| Postgres `23503` · SQLite FOREIGN KEY | `ForeignKey` **409** |
| Postgres `23502` / `23514` · SQLite NOT NULL / CHECK | `DatabaseError` `not_null` / `check` **422** |
| Postgres `40001` / `40P01` | left thrown so `flow.retry` can run; after exhaust → `ServiceUnavailable` **503** |
| `08xxx` / `53300` / connection / paused | `ServiceUnavailable` **503** |
| `42P01` / `42703` with `domainDdl: "off"` | **OKE1110** (unchanged) |
| other SQLSTATE | `DatabaseError` `unknown` **500** |

Files / KV `get` returning `null` is **not** auto-`NotFound` — return `fx.fail.notFound` yourself.
...
<Accordion title="Unique insert is 500 InternalError">
  Unique / exclusion maps to `Conflict` **409**; missing FK to `ForeignKey` **409**. Serialization
  (`40001` / `40P01`) stays thrown until retries exhaust, then `ServiceUnavailable` **503**.
</Accordion>
