...
| `NotFound`             | Missing row / object — **404** JSON envelope (router miss is plain-text `404`)                  |
| `Conflict`             | Unique / exclusion — **409** (SQL unique auto-maps here)                                       |
| `ForeignKey`           | Foreign-key violation — **409**                                                                |
| `DatabaseError`        | Other SQL constraints — `not_null`/`check` **422**, `retryable` **503**, else **500**          |
| `ServiceUnavailable`   | Connection / exhausted serialization — **503**                                                 |
| `InternalError`        | Unhandled throw — **500**, catalog message only                                                |
...
| `AuthFailed`           | Credentials / policy — **400**                                                                 |
| `AuthRateLimited`      | Auth attempts exhausted — **429**                                                              |
