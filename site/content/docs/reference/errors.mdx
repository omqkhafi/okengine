...
<Callout title="Effects are usually inferred">
  The 1001–1007 · 1008 · 1009 family exists for flows that declare effects explicitly.
  Most apps never write an `effects` block — inference covers them — so seeing one of
  these means an explicit declaration drifted from the code.
</Callout>
