...
<Callout title="The one rule">
  Switch on `error.code` for typed `data`. Use `explain().kind` for chrome when the code is
  unknown. Catch `OKE####` only for thrown framework invariants.
</Callout>
...
```typescript
import { explain } from "okengine/client/explain";
...
`import { explain, matchError } from "okengine/client/explain"`. Do not switch on every code — a
`create` can return store auto-map, gates, transport, and domain codes.
...
`_` is required. `explain(error).fields` maps `ValidationError` issues to `{ email: "Required" }`.
See [Calling](/docs/client/calling).
...
<Accordion title="Unique insert is 500 InternalError">
  Unique / exclusion → `Conflict` **409**; missing FK → `ForeignKey` **409**; invalid → `DatabaseError` **422**.
  Serialization (`40001` / `40P01`) stays thrown until retries exhaust, then **503**.
  See [SQL auto-map](#sql-auto-map).
</Accordion>

<Accordion title="Redis down returns InternalError">
  Connection / `CLUSTERDOWN` / `READONLY` map to `ServiceUnavailable` **503**.
  `BUSY` / `TRYAGAIN` stay thrown until `flow.retry` exhausts, then **503**.
  `WRONGTYPE` stays `InternalError` — see [KV auto-map](#kv-auto-map).
</Accordion>

<Accordion title="S3 AccessDenied / disk full is InternalError">
  `AccessDenied` and `fs` path-escape keys → `Forbidden` **403**.
  `SlowDown` / `EBUSY` stay thrown until retries exhaust, then **503**. `ENOSPC` / `NoSuchBucket` → **503**.
  Missing `get` is still `null`, not `NotFound` — see [Files auto-map](#files-auto-map).
</Accordion>
...
- [Calling](/docs/client/calling) — `explain` / `matchError` on the client
