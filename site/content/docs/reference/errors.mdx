...
<Accordion title="OKE1072 Signal or Clock flow unnamed">
  Cause: `A {kind} flow on "{trigger}" has no name.` Nameless `flow({ do })` is the tree default.
  Outside a unit folder, pass `flow("unit.export", {…})`. See [Signal · Flow
  name](/docs/elements/signal#flow-name) · [Clock · Flow name](/docs/elements/clock#flow-name).
</Accordion>
