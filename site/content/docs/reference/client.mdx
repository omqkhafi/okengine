...
The app pipeline accepts **Bearer** access tokens (`Authorization: Bearer …`). Gates decide on
that principal — they do not log anyone in. Your Flows (backed by `okengine/auth` sessions)
return tokens to the UI; the client only attaches and refreshes the access token.
...
const api = createClient(app, "http://localhost:6530", {
  auth: {
    ...
    refresh: async () => {
      // Call a Flow *you* wrote that rotates the refresh token (okengine/auth
      // has rotateRefresh — there is no built-in app refresh route).
      // Update accessToken before returning; the return value is unused.
...
Not in `okengine/client` today: cookie jars, session React hooks, or a merged plugin `.client()`
helper.

A planned `okengine/client-react` package will hold hooks without growing the core client past
its size budget. For browser apps also see [CORS](/docs/plugins/cors) and
...
return value is ignored. There is no built-in app refresh route; wire a Flow that rotates the
refresh token, or re-login when rotation fails.
