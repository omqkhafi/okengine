...
const stop = api.live(
  ...
  { orderId: "ord_1" },
  {
    ...
    autoResubscribe: false, // default — true reopens after a drop (500ms…30s backoff, full replay)
  },
);
...
v1 ignores SSE `id:` (no Last-Event-ID resume). Fetch carries Bearer auth; no timeout, no retry.
`autoResubscribe: true` reopens after a drop with 500ms exponential backoff, capped at 30s.
