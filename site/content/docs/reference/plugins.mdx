...
<Accordions type="single">
  ...
  <Accordion title="Boot: plugin boot failed — unmet .needs() dependencies">
    ...
    nothing satisfied it. Plug the named peer plugin, or ensure the element/driver is available at
    boot (tables imply `store.sql`; declared clocks imply `clock`, and so on).
  </Accordion>
</Accordions>
