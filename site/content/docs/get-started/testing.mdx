...
<Callout title="The one rule">
  Boot the same `app` your server boots. Assert only through the harness
  surfaces (`t.api`, `t.auth`, `t.signals`, `t.clock`, `t.channels`,
  `t.ai`, `t.effects`) — never reach into internal runtimes.
</Callout>
