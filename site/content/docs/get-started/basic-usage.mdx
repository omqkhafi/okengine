...
For Bearer sessions today, pass `auth: { getToken, refresh }` on
`createClient` options. Reactive session hooks and framework bindings are
**not** in `okengine/client` (hard &lt;3 kB budget) — the planned shape is a
framework-agnostic session adapter, then an opt-in `okengine/client-react`
subpath (`SessionProvider` / `useSession`). Plugin `.client()` mirroring
stays deferred until that seam merges into `createClient`.
