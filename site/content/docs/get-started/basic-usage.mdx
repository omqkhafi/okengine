...
Mount the same app under a host path — gates and effects stay enforced:
...
- [ ] Optionally mount into an existing Hono/Express host via `mount()`
- [ ] Optionally export Runs to OpenTelemetry (`createRunsRuntime({ otel })`) — additive

## What's next

<Cards>
  <Card title="Deploy" description="Prod protocols, platform env, staging as a second copy." href="/docs/get-started/deploy" />
