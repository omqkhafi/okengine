...
schema or codegen project. For Bearer sessions, pass `auth: { getToken, refresh }`.

Reactive hooks and framework bindings are **not** in `okengine/client` (hard
&lt;3 kB budget). See [Client](/docs/reference/client) for REST vs RPC, options,
and helpers.
