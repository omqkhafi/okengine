...
or docker first; `store.index` as a normal facet with `none`; **AI setup** =
Recommended / Customize / Off; saved to `~/.oke/create-defaults.json`), or
**reuse** matching saved settings. Project name rejects a non-empty existing
path.
