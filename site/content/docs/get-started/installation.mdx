...
special, or **Generate** (copies it). Flows listed means the install worked —
...
<Accordion title="Create first operator fails (password / claim code)">
  Password: ≥ 12 chars with upper, lower, number, and special — or **Generate** to fill and copy.
  Copy the claim code from the `oke dev` board (30 min TTL; restart mints a new
  one), or run `oke console claim-code`. The form never receives the code over
  HTTP. The form and request log show the reason — not a bare `TransportError` /
  `500`.
</Accordion>
