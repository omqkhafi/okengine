...
The okengine repo also ships `examples/keel` — a Linear-shaped app, not a template.
Run it with `bun run dev:keel` against Compose (Postgres, Redis, RustFS, Mailpit, Meilisearch).
...
<Accordion title="Create first operator fails (password / claim code)">
  Password: ≥ 12 chars with upper, lower, number, and special. Copy the claim code from the current
  `oke dev` boot log (30 min TTL; restart mints a new one). The form and request log show the reason
  — not a bare `TransportError` / `500`.
</Accordion>
