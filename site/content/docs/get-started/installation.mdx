...
<Accordion title="Create first operator fails (password / claim code)">
  Password: ≥ 12 with upper, lower, number, and special — or **Generate**.
  Copy the claim code from the `oke dev` board (30 min TTL) or `oke console claim-code`.
  The form never gets the code over HTTP; the log shows the reason, not a bare `500`.
</Accordion>
...
<Accordion title="Want a clean keel stack?">
  Stop `bun run dev:keel`, then `bun run dev:keel:reset`. That wipes Compose
  (`oke docker clean --yes`), `.oke/`, Docker extras, and writes `.env.local` from
  `.env.example`. Next `dev:keel` regenerates stack keys; then `bun run dev:keel:seed`.
</Accordion>
