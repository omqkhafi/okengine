...
export const webhookSecret = vault.secret("APP_WEBHOOK_SECRET", { dev: "dev-webhook-secret-change-me" });
export const noteCreatedMail = channel.email({ from: "Notes <notes@localhost>" }).template("note-created", {
  locales: ["en"],
...
export const create = on(
  ...
  flow({
    ...
    do: async (input, fx) => {
      ...
      await fx.store(db).insert(notes).values({ id, ...input, archivedAt: null, createdAt });
...
export const get = on(
  ...
  flow({
    ...
    do: async ({ id }, fx) => {
      ...
      return { id: String(row.id), title: String(row.title), body: String(row.body), archivedAt: null, createdAt: Number(row.createdAt) };
    },
  }),
);
```
...
export const list = on(
  ...
  flow({
    ...
    do: async (input, fx) => fx.json.withQuery(await fx.store(db).select().from(noteComments), input),
  }),
);
```
...
```typescript title="flows/my/index.ts"
export const tasks = on(http.get("/me/tasks").gate(member), flow("my.tasks", { do: async () => [] }));
```
...
export const bySlug = on(http.get().public(), flow({ in: z.object({ slug: z.string() }), do: async ({ slug }) => ({ slug }) }));
...
<Cards>
  <Card title="Flow" description="The one species of backend behavior." href="/docs/elements/flow" />
  <Card
    title="Basic Usage"
