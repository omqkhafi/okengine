...
**Consequence:** the live route re-checks RLS for every CDC event under the subscriber's stamped identity. A row that leaves the caller's visibility window arrives as `{ kind: "revoked", id, reason: "rls" | "query" }` — never as a silent stale upsert. Clients consume it with `useLiveQuery` from `okengine/client-react`.

#### Hand-written live flows

`store.resource` is optional. A hand-written flow declares the same live
surface with the `.live(table)` trigger modifier and the `liveQuery` helper —
identical signal, guardrails, and per-subscriber classification:
...
`.live(tasks)` stamps the internal `oke/live/sql:tasks` signal at extract and
enforces the same guardrails as `live: true` (PK required; `updatedAt` / RLS
warnings).

**Consequence:** the flow body parses the request's query params with the
exact list grammar, then opens the classified CDC stream.

- `?status=eq.open` filters the live window the same way it filters
  `GET /tasks`.
- A flow that also writes other tables (an activity log alongside the main
  write) never leaks those tables into the stream: classification is
  per-table.

`live: true` carries compile-time guardrails at extract, plus durable delivery
and runtime watchtowers:

- A table with **no primary key fails extract loud** — upsert/revoked/delete
  address rows by PK.
- Missing `updatedAt`/`updated_at` warns (no `rowVersion` LWW guard; races
  rely on `X-Oke-Mutation-Id` echo dedupe), missing RLS policies warns
  (every gated subscriber would see every row).
- CDC events ride the durable `oke_cdc_outbox`; delivered rows prune on
  retention defaults (24h / 50k) so the outbox never grows unbounded.
- `oke doctor` findings watch the runtime side: `cdc_outbox_backlog`,
  `cdc_outbox_retention`, `live_subscriber_pressure` (~150 subs/table in v1),
  and `live_fanout_queue_saturated`.
