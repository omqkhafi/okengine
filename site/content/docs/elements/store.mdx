...
<Callout title="Not per-key, not Flow durable, not auto-cache">
  Durable KV is a JSONB table on the same Postgres as `store.sql`. Flow `durable` journals
  steps. Tier-1 auto-cache is an in-process Map and never reads `store.kv`.
</Callout>
