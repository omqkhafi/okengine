...
<Callout title="Console Files is a folder browser">
  Inspect the **bucket**. Click a file to preview in a sheet. Uploads
  use a URL-safe key; the original name lives in the object catalog.
  Bytes live on `memory`, `fs`, or `s3`.
</Callout>
