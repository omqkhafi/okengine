...
| `OKE_STORE_KV_DURABLE_URL`  | Required when any `store.kv` is `{ durable: true }`     |
| `REDIS_DURABLE_URL`         | Alias if `OKE_STORE_KV_DURABLE_URL` is unset            |
...
Default `store.kv("sessions")` is cache-shaped — a Redis recreate drops keys. Opt in with a
**second namespace** on a second URL. Not a per-key flag: Redis AOF is server-wide.
...
`{ durable: true }` binds that namespace to `OKE_STORE_KV_DURABLE_URL`. Gate rates and Signal stay
on `REDIS_URL`. Missing durable URL fails boot: `oke boot: durable store.kv needs OKE_STORE_KV_DURABLE_URL`.

<Callout title="Not per-key, not Flow durable, not auto-cache">
  Persistence is server-wide — two stores, two servers (or two managed databases). Flow `durable`
  journals steps. Tier-1 auto-cache is an in-process Map and never reads `store.kv`.
</Callout>

Self-hosted Redis/Valkey AOF is `appendonly yes` + `appendfsync everysec` (~1s RPO) when at least
one durable store is declared. The durable volume is always in compose; AOF flags stay off until
then. Dragonfly durable is snapshot-scheduled — boot warns. See [Redis](/docs/recipes/redis).
