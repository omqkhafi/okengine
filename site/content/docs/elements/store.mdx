...
<Callout title="The one rule">
  Drivers are named after **protocols**, not vendors (`postgres`, `redis`, `s3` — never
  `neon` or `minio`). Vendor choice lives in the `images` map of `oke.config.ts`.
</Callout>
...
`.gate(...)` / `.public()` chain like `http.get` — gates on every verb. Pass `live: true` to also mount `GET <path>/live` — an SSE feed of per-subscriber classified events (`upsert` / `revoked` / `delete`), gated the same way as the CRUD verbs.
...
The live route re-checks RLS for every CDC event under the subscriber's stamped identity. A row that leaves the caller's visibility window arrives as `{ kind: "revoked", id, reason: "rls" | "query" }` — never as a silent stale upsert. Clients consume it with `useLiveQuery` from `okengine/client-react`.

`store.resource` is optional. A hand-written flow declares the same live surface with the `.live(table)` trigger modifier and the `liveQuery` helper — identical signal, guardrails, and per-subscriber classification:
  ...
  flow("tasks.live", {
    ...
    do: async (input, fx) =>
      liveQuery(fx, tasks, input, {
        search: [tasks.title],
        filter: [tasks.status], // same PostgREST grammar as list
        order: "all",
      }),
  }),
);
...
Helpers emit stable names (`gate_member_select`). Raw `store.schema.policy("name", { as, to, for, using, withCheck })` is the escape hatch — predicates use `oke.*`, never `current_setting`.
...
export const daily = store.schema.table("daily", {
  id: field.text().primaryKey(),
  code: field.text().notNull().references(() => links.code),
...
export default defineSeed({
  ...
  essential: async (fx: Fx) => {
    await fx.store(db).upsert(notes, { id: "welcome" }, { id: "welcome", title: "Welcome", body: "…", createdAt: 1 });
  },
  dev: async (fx: Fx) => {
    await fx.store(db).upsert(notes, { id: "sample-ideas" }, { id: "sample-ideas", title: "Ideas", body: "…", createdAt: 2 });
  },
...
await fx.store(db).upsert(notes, { id: "welcome" }, { id: "welcome", title: "Welcome", body: "Your Notes API is ready.", createdAt: 1 });

// Refresh copy on every seed — match stays; title/body are rewritten
await fx.store(db).upsert(notes, { id: "welcome" }, { id: "welcome", title: "Welcome (updated)", body: "New bootstrap copy.", createdAt: 1 }, { onExisting: "update" });
```
...
<Callout title="Licenses bite managed-service resellers — read this once" type="warn">
  RSAL, SSPL, and BSL service restrictions apply when you offer that specific datastore **as a service** to third parties — not when you run it for your own app. Pick for your situation; OKE does not pick a "safer" default for you.
</Callout>

<Callout title="Dry-run refuses KV writes">
  `set` and `delete` throw `DryRunWriteIsolationError` during dry-run — the runtime will not risk a double-write against a shared Redis. Reads (`get` / `list`) still run.
</Callout>
...
A `store.schema.table` column named `name` used to shadow the SQL table name. Inserts and selects now resolve it from the declaration. Keep the `name` column.
</Accordion>
