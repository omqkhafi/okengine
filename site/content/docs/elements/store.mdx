...
<Callout title="Query performance">
  The SQL band **Performance** view reads `pg_stat_statements` and live locks
  (`QUERY /console/store/sql/stats` and `/locks`). Alpine needs preload plus
  `CREATE EXTENSION`; Index Advisor is opt-in on [Postgres](/docs/recipes/postgres#query-performance).
</Callout>
