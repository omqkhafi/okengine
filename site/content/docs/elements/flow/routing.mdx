...
<Callout title="The one rule">
  On a tree file, omit the path (`http.get()`). The file tree stamps the URL and
  `unit.export`. The HTTP **method** still comes from the trigger. An explicit
  `http.get("/x")` always wins.
</Callout>
...
Routes use an explicit path or inherit one from the file tree.

**Explicit path** — pass the URL template; the tree never overwrites it:

```typescript
http.get("/organizations/:orgId/members/:memberId");
```

**Pathless** — omit the path so `generated.ts` stamps the URL from disk:
...
| ------- | ------ | ------- |
| HTTP path | File tree or explicit `http.get("/x")` | `/notes/:id` |
| Flow name | `flow("notes.get")`, else `unit.export` | `notes.get` |
| Client | Unit folder + `export const` | `api.notes.get({ id })` |

Nameless `flow({ do })` is the tree default. Explicit `flow("notes.get")` wins
when the prefix matches the folder.
...
- [Client](/docs/reference/client) — `api.notes.get`, REST vs RPC, `$routes`
- [Errors](/docs/reference/errors) — OKE1010 · OKE1009 · OKE1011 · OKE1012
- [Manifest](/docs/reference/manifest) — derived routes, no hand-written table
