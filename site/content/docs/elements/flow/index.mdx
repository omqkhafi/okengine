...
<Accordion title="OKE1008 — no declared effects">
  ...
  Boot with `oke build` / `oke dev` so extract can infer. Framework / plugin Flows
  outside the app tree get an empty least-privilege stamp automatically when a
  Manifest is present — do not hand-write `effects: {}`. Real `fx` usage still
  needs inference (app source) or an explicit non-empty `effects` block when
  extract cannot see the body. If extract failed (`Manifest extract failed`),
  install `oxc-parser` and retry.
</Accordion>
