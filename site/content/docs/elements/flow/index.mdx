...
<Accordion title="Name stamping">
  Prefer nameless `flow({ do })` on tree files — `src/flows/hooks/inbound.ts` +
  `export const ingestWebhook` stamps `hooks.ingestWebhook`. Signal / Clock nameless
  outside `src/flows/<unit>/` fails **OKE1072** ([Signal · Flow name](/docs/elements/signal#flow-name)); HTTP is **OKE1045**.
</Accordion>
