...
<Callout title="Call-only Flows">
  A Flow with no trigger — just `flow("payments.charge", { … })` — is internal. Nothing outside
  your code can start it. Other Flows invoke it with `fx.call(flowRef, input)`.
</Callout>
...
| ------- | ---- | ----------- | ---------- |
| HTTP | `on(http.get(path), flow)` | A request | Merged path / query / body |
...
| Call-only | `flow(name, { … })` or `on(internal, flow)` | `fx.call` | Callee `in` |
| MCP | `on(mcp.tool("x").gate(…), flow)` | MCP `tools/call` | Tool args |

`delivery: "live"` is an HTTP SSE tape — bind it with [`http.live`](/docs/elements/flow/http#live-streams), not as a worker.
...
<Accordion title="Name stamping">
  `flow({ do })` starts nameless. The file tree, `.adopt()`, or an explicit
  `flow("notes.create", { … })` stamps `unit.export`. HTTP Flows that stay nameless fail
  **OKE1012**. See [Routing · Names](/docs/elements/flow/routing#names).
</Accordion>
...
<Accordion title="OKE1008 — no declared effects">
  ...
  Deploy-shaped environments refuse an open capability token. If extract itself
  failed (boot warns `Manifest extract failed`), install `oxc-parser` (okengine
