...
| Clock      | `on(clockDecl, flow("name", { do }))`     | Interval or cron tick                       | none (`_`)                             |
...
Declare the Signal or Clock as an exported const, then bind
`on(handle, flow("name", { do }))`. Missing name is **OKE1072**; collision is **OKE1070**.
...
<Accordion title="Schema at emit">
  ...
  consumer runs. This is an **emit** contract — workers inherit the payload from the Signal;
  they do not declare `in` on `flow()`.
</Accordion>
...
<Callout title="Detailed section">
  ...
  handle with `on(clockDecl, flow("name", { do }))`.
</Callout>
...
<Accordion title="OKE1070 — flow name defined twice">
  Cause: `Flow "{flow}" is defined twice.` Two `flow("…")` strings share a name. Give at least
  one a distinct name.
</Accordion>
