...
<Callout title="Detailed section">
  Prefer `clock.every` / `clock.daily` / `clock.cron` (and the other named helpers) for
  fixed schedules — they compile to the same decl as the bare callable. The lower-level
  `clock(name, { cron | every })` form still works when you need a raw expression or a
  schedule chosen programmatically; missing both throws
  `clock("name"): require cron or every`. Bind the returned handle with `on(clockDecl, flow)`.
</Callout>
