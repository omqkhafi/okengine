...
<Callout title="Bind and emit are independent">
  The consumer and the producer share one Signal handle. They can live in
  different files, written in any order — emit is not "step 2" after bind.
</Callout>
...
<Accordion title="OKE1071 — once signal bound to more than one Flow">
  ...
  Flow. See [Once · Competing consumers](/docs/elements/signal/once#competing-consumers-once-vs-broadcast).
</Accordion>
