...
[Clock](/docs/elements/clock#inline-or-named-export). Flow name (explicit / tree):
[Signal](/docs/elements/signal#flow-name) · [Clock](/docs/elements/clock#flow-name) — **OKE1070** / **OKE1072**.
...
<Accordion title="OKE1070 — flow name defined twice">
  Cause: `Flow "{flow}" is defined twice.` Two explicit `flow("…")` calls share a name, or two tree
  exports stamp the same `unit.export`. Give at least one an explicit `flow("…")` or a distinct
  tree export.
</Accordion>
