...
<Callout title="Detailed section">
  ...
  schedules — they compile to the same decl as the bare callable. The lower-level bare
  `clock(name, options)` form still works when you need a raw expression or a schedule chosen
  programmatically; missing both `cron` and `every` throws
