...
`http.get` · `http.post` · `http.put` · `http.patch` · `http.delete` · `http.head` · `http.options` · `http.query`. QUERY (RFC 10008) is a safe, idempotent read that still carries a JSON body — same parse path as POST.
...
The router returns **405 Method Not Allowed** with `Allow` listing every method bound to that path. A path that no flow owns is still 404.
