...
**Crash recovery needs a shared journal.** Today's durable journal is in-process memory only, with no boot-time recovery scan. The replay guarantee is proven for the same process only — not after a real crash/restart, and not across load-balanced instances.
