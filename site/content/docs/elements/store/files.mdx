...
<Accordion title="S3 AccessDenied / disk full is InternalError">
  `AccessDenied` / `EACCES` map to `Forbidden` **403**. Missing `get` stays `null`.
  `SlowDown` stays thrown until `flow.retry` exhausts, then **503**.
  `NoSuchBucket` / `ENOSPC` are **503** — see [Errors · Files auto-map](/docs/reference/errors#files-auto-map).
</Accordion>
