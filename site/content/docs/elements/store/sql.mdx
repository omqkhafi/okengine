...
Seed never runs at boot without confirmation — only via `oke db seed`, the live
`oke dev` **`s`** key, or the first-boot confirm when a seed module exists and
`.oke/state.json` has not recorded that identity yet. Categories:
