...
<Callout title="Detailed section">
  If you only need BM25, skip this. Fusion runs only when both BM25 and LSH hit
  lists exist — then ranks are fused and truncated to `limit`.
</Callout>
...
<Accordion title="No vector hits">
  When LSH produces no scored neighbors (or query embedding is unwired), order is
  pure BM25. `fusedBy` / `rrfK` are omitted. `meta.engine` may still list `"lsh"`
  if the table declared `.embed()` columns.
</Accordion>
...
<Accordion title="CLI not fully wired yet">
  The subcommand is registered (`<table>`, `--batch` default 32) and never
  auto-runs on push. Until a live SQL connection is wired for the project, the
  CLI prints the programmatic path:
...
<Accordion title="meta.engine lists lsh but fusedBy is missing">
  Schema has `.embed()` columns, so `meta.engine` includes `"lsh"`. Fusion only
  runs when query + stored vectors produce scored neighbors. Check that
  `embedQuery` is wired at boot and that backfill / CDC wrote embeddings.
</Accordion>
