...
| Style                                         | When                                                                                       |
| --------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `on(clock.every("name", "1h"), flow({ do }))` | One file. Nameless `flow({ do })` only under `src/flows/<unit>/` — [Flow name](#flow-name) |
...
<Callout title="Why this nameless flow({ do }) is legal">
  No `flow("name", …)` string — the Flow is **`health.pingExternal`** from
  `src/flows/health/ping.ts` + `export const pingExternal` ([Flow name](#flow-name)).
...
Nameless `flow({ do })` is the tree default — Inline `pingExternal` is `health.pingExternal`
with no `flow("…")` string. Outside a unit folder it fails **OKE1072** (HTTP: **OKE1045**;
collision **OKE1070**). Multiple Clock consumers are allowed — each needs its own name.
...
the Clock name `cleanup` — same tree rule as the Inline tab:
...
<Accordion title="OKE1072 — Clock flow unnamed">
  ...
  Two valid names: `flow("unit.export", {…})`, or a tree export (`src/flows/health/ping.ts`
  + `export const pingExternal` stamps `health.pingExternal`). Same code outside
