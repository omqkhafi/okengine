...
| `breachCheck`                          | unset     | Optional HIBP (or custom) check → `AuthFailed` / `password_breached`           |
...
chain. Rate gates with `overridable: true` can be retuned live.
...
Declare the rate with `overridable: true` and redeploy — live retuning is opt-in.
