...
SQL stamps `oke.tenant()` when tenancy is on — [Store](/docs/elements/store).
...
<Callout title="RLS reads the stamped principal">
  ...
  `oke.has_scope()`. With `gate.auth.tenant` on, a fourth GUC `oke.tenant()` is set.
  Rate, cron, CDC, and signal stay unstamped. Helpers: [Store](/docs/elements/store).
</Callout>
