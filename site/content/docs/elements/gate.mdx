...
`gate.public` is the explicit public sentinel. It is always registered
in the Gate runtime — attach it on the trigger; you do not list it in
`oke({ gates })`. Every HTTP trigger must carry a gate or `gate.public`
at boot — omit neither (see below).
