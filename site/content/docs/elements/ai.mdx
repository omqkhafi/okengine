...
export const smart = ai.model("smart", {
  provider: "ollama",
  model: "qwen3:8b", // balanced local-dev starting point — override freely
});
```

- **Model:** set on `ai.model(…, { model })` or `OKE_AI_MODEL`. `qwen3:8b` is a reasonable balanced starting point for local dev — not the only option.
...
Docker mode expects `OKE_AI_URL` from `docker/.env.docker` (written when `images.ai` is pinned). Locally, the default is `http://127.0.0.1:11434`. If asks fail with `OllamaUnavailableError`, confirm the server answers and that you have pulled the model you named (`ollama pull qwen3:8b` or your `OKE_AI_MODEL`).
