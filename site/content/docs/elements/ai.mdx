...
Ollama: machine banner (OS · CPU · RAM · Fit RAM · detected models · tiers) →
**Select model** (Ultra Fast / Fast / Balanced / Smart → Use recommended or
Select manually ≤10 with modalities) or **Manual model**. Embed defaults to
`nomic-embed-text` (auto-pull when missing — no download prompt). Cloud:
API token → Select model (≤10) or Manual model.

`create-oke` customize offers **AI setup** (Recommended Ollama · Customize ·
Off) before install; `--ai` forces the full model wizard. Standalone
`oke ai setup` remains for existing apps.
