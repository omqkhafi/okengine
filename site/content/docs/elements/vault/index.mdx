...
<Accordion title="VaultBootError — N missing secret(s)">
  Cause: `vault boot failed — N missing secret(s):` with every gap listed. On a
  TTY, `oke dev` prompts each into `.env.local`; otherwise use `oke vault set` /
  env / managed, or a `dev:` fallback. `vault.env.required` joins the same list.
</Accordion>
