...
<Accordion title="VaultBootError lists this secret">
  No resolution layer supplied a value. On a TTY, `oke dev` prompts into
  `.env.local`; otherwise use `oke vault set` / env / managed, or a `dev:`
  fallback. Per-tenant contracts are skipped at boot — seed the tenant path.
</Accordion>
