...
<Callout title="Builtin concurrency (rotate / audit / backup)">
  `rotate-master` uses SKIP LOCKED + lease TTL (one holder; crashed holders reclaim). Audit
  appends under a transactional row lock. Backups use temp → fsync → rename with end marker +
  checksum — restore rejects incomplete files before decrypt.
</Callout>
