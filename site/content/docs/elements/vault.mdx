...
<Callout title="RLS on ciphertext rows">
  `tenant_id = oke.tenant() OR tenant_id IS NULL`. ENABLE without FORCE: the adapter
  (owner) still sees every row; Console / `oke_app` cannot list another tenant's
  ciphertext. Global `NULL` rows stay visible to every stamped tenant.
</Callout>
