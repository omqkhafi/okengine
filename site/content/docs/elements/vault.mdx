...
Yes — this example only. `oke db seed` (dev) writes the nine keel contracts
into the built-in store. `.env.local` keeps stack URLs and the master key;
other apps leave Vault empty.
