...
The **Console** (`:6533`) **Vault** page (`/vault`) is write-only: a salted
fingerprint (`sha256:…`) per secret, never the cleartext.
...
| Action               | Confirm                                | HTTP                                                                   |
| -------------------- | -------------------------------------- | ---------------------------------------------------------------------- |
| Set / rotate a value | `SET` (production) / `ROTATE` (always) | `POST /console/vault/set` · `/rotate`                                  |
...
radius before you confirm rotate, and `is:overdue` when a secret is past its
`rotate: "90d"` hint.
