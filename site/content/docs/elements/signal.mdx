...
do: async (input, fx) => {
  ...
  // fx.emit commits the signal outbox when the call resolves (postgres → oke_signal_messages).
};
```
...
| `postgres` | your existing Postgres container | Default for docker/prod — durable outbox table `oke_signal_messages` |
| `redis`    | Redis container                  | Explicit alternative when throughput outgrows Postgres               |
| `nats`     | NATS container                   | High-throughput fan-out                                              |

`postgres` keeps committed emits in `oke_signal_messages`. `fx.emit` finishes that outbox write when the call resolves; consumers claim with `FOR UPDATE SKIP LOCKED` and lazy lease reclaim. Pairing an arbitrary Store insert with emit inside **one** shared SQL transaction is not what `fx.emit` does today — keep consumers idempotent (at-least-once).
...
`fx.emit` commits the signal outbox when it resolves; it does not wrap your Store insert. Treat delivery as at-least-once and make the consumer idempotent (or check the row before acting). Prefer `postgres` when you want a durable outbox table rather than in-process memory.
