...
Server-side `fx.live` **is** the SSE carrier (`JsonStreamResult`). Custom `do` is allowed; authors do not wrap `fx.json.stream`. v1 replays the **full** retained tape (no Last-Event-ID resume, no TTL). `autoResubscribe: true` opens a new request after a drop — not cursor resume.
