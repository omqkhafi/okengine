...
Same signal, two audiences: member filter on `:orderId`, admin firehose. Duplicate `(signal, gates, match)` is **OKE1013**. Same HTTP path is still **OKE1011**.

Firehose default: `on(http.live(orderStatus).gate(member))` → `GET /_oke/live/order-status`.

Server-side `fx.live` **is** the SSE carrier (`JsonStreamResult`). Custom `do` is allowed; authors do not wrap `fx.json.stream`. v1 replays the **full** retained tape (no Last-Event-ID resume, no TTL). `autoResubscribe: true` opens a new request after a drop (500ms exponential backoff, 30s cap) — not cursor resume.
...
`live` retains every delivered message and replays the **full history** to a late subscriber. There is no TTL or max-count window on that retention today (the Console payload monitor shows only the newest 50 for display). Last-Event-ID resume is deferred — the wire already carries `id:`.
