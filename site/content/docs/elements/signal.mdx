...
<Callout title="Client subscription is not shipped yet">
  `delivery: "live"` means the **driver** retains and replays. Listen with
  `bus.live()` on the server — not `on(signal, flow)`. `createClient` has no
  SSE yet — poll HTTP. See [Client](/docs/reference/client#signal-and-live-queries).
</Callout>
