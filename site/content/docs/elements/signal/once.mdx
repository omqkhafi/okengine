...
| ------ | ---- | ------- | ------- |
| `schema` | Standard Schema | omitted | Validated at `fx.emit` (**OKE1250**) |
...
<Accordion title="Schema at emit">
  Invalid payloads fail at `fx.emit` with **OKE1250** before any worker runs. Same Standard
...
<Accordion title="Worker never runs after emit">
  ...
  adopted into the app. Check **OKE1240** if the emit itself threw. Live Signals are not
