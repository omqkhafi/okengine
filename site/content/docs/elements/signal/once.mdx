...
<Callout title="One handle, three independent uses">
  `emailTask` is a shared const. Declare it, bind a worker, and emit — different
  files, any order. Binding is not "step 2" after declare; emit is not "step 3".
</Callout>
...
<Accordion title="Both of two Flows ran on one once message">
  That is broadcast physics, not once. Two different Flow definitions on one `once` signal now
  fail **OKE1071** at boot. If you need every Flow to run, switch the declaration to
  [`signal.broadcast`](/docs/elements/signal/broadcast).
</Accordion>

<Accordion title="OKE1071 — once signal bound to more than one Flow">
  Cause: `Once signal "{signal}" is bound to more than one Flow ({flows}).`
  Use `signal.broadcast` if each Flow should independently receive this event, or bind only one
  Flow. Replicas of one Flow are still one `on()` in source.
</Accordion>
