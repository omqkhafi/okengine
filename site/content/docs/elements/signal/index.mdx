...
<Callout title="One handle, three independent uses">
  `orderPlaced` is a shared const. Declare it, bind a worker, and emit from a
  producer — different files, different people, any order. None of these is a
  prerequisite step for the others.
</Callout>
...
<Accordion title="OKE1071 — once signal bound to more than one Flow">
  ...
  Flow. See [Competing consumers](#competing-consumers-once-vs-broadcast).
</Accordion>
