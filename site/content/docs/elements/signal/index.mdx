...
| Style                                         | When                                                                                          |
| --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `on(signal.once("name", opts), flow({ do }))` | One file. Nameless `flow({ do })` only under `src/flows/<unit>/` — [Flow name](#flow-name)    |
...
<Callout title="Why this nameless flow({ do }) is legal">
  No `flow("name", …)` string — the Flow is **`hooks.ingestWebhook`** from
  `src/flows/hooks/inbound.ts` + `export const ingestWebhook` ([Flow name](#flow-name)).
  The same `flow({ do })` anywhere not under `src/flows/<unit>/` fails **OKE1072**.
</Callout>
...
| Style                         | When                                                         |
| ----------------------------- | ------------------------------------------------------------ |
| `flow("orders.fulfill")`      | Manifest / `fx.call` name — required outside a unit folder    |
| Tree `export const onCreated` | `src/flows/notes/on-created.ts` stamps `notes.onCreated`     |

Nameless `flow({ do })` is the tree default — Inline `ingestWebhook` is `hooks.ingestWebhook`
with no `flow("…")` string. Outside a unit folder it fails **OKE1072** (`A signal flow on
"{trigger}" has no name.`). HTTP nameless after adopt fails **OKE1045**; collision is **OKE1070**.
...
not `note-created` — same tree rule as the Inline tab:
...
<Accordion title="OKE1072 — Signal flow unnamed">
  ...
  Two valid names: `flow("unit.export", {…})`, or a tree export (`src/flows/hooks/inbound.ts`
  + `export const ingestWebhook` stamps `hooks.ingestWebhook`). Same code outside
  `src/flows/<unit>/` fails — [Flow name](#flow-name).
</Accordion>
