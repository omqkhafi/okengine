...
```typescript title="src/flows/catalog/update.ts"
import { on, flow, http } from "okengine";
import { z } from "zod";
import { cacheInvalidated } from "@/signals/cache";

export const updateSku = on(
  http.patch("/skus/:sku"),
  flow("catalog.updateSku", {
...
```typescript title="src/flows/catalog/write.ts"
import { on, flow, http } from "okengine";
import { z } from "zod";
import { catalogChanged } from "@/signals/catalog";

export const writeCatalog = on(
  http.post("/catalog"),
  flow("catalog.write", {
    in: z.object({ sku: z.string(), title: z.string().min(1) }),
    do: async ({ sku, title }, fx) => {
