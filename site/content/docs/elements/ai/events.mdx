...
The response is `text/event-stream`. Pass your own `threadId`. The default is this process's run id (`agent-run-1`, …). Those ids collide across processes and after a restart.
...
Approving or denying does not continue this SSE response. The scheduler resumes the durable run in the background and replays the tool once. Decode the approval id. The prefix before the first `.` is the durable Flow run id. After that run's status is `completed`, its journal `output` is the agent result (`stopReason`, `output`, `cost`, and the trail). It is not `{ streamed: true }`. This stream will not yield it.
...
Pass `threadId`. The default ids (`agent-run-1`, …) collide across processes and after a restart.
