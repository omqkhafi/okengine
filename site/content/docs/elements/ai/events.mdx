...
`GET /agent/runs/:runId/events` is `text/event-stream`. Each frame has an `id`. Send that id as `Last-Event-ID` to resume. The route checks the same gate and tenant as the Flow that started the run.
