...
The response is `text/event-stream`. Pass `threadId` when the chat already has an id. The default is this process's run id (`agent-run-1`, …), so two processes do not share a thread.
...
Approving or denying does not continue this SSE response. The scheduler resumes the durable run in the background and replays the tool once. Decode the approval id to get the run id, then read that run's `output` after it finishes. That is the resumed result. This stream will not yield it.
...
Pass `threadId`. Otherwise each process mints its own (`agent-run-1`, …) and a restart starts over.
