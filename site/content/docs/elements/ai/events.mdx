...
The run stream yields every token. The follow log coalesces text and argument deltas (about every 100 ms or 1 KB, including a timer) and stores structural events one by one. Past the cap, only `RUN_FINISHED`, `RUN_ERROR`, and interrupts remain. A finished run's events are deleted after 24 hours by the scheduler. The log is stored on the journal driver and keyed by the agent run id. A follower must be the starting principal or an operator, and must pass every gate on the Flow.
