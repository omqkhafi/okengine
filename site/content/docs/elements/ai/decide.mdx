...
<Callout title="Experimental">
  `ai.decision` is experimental. A promoted lockfile applies after restart. Drift is one flag for the whole app. A seed file can print a certificate. It is not a production pilot. Score levels and a whole question held in a same-file const are checked at compile time.
</Callout>
