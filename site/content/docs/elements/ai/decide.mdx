...
<Callout title="Experimental">
  `ai.decision` is experimental. A lockfile grants an automatic answer only after labels exist. A seed file can print a certificate. It is not a production pilot.
</Callout>
...
The park waits until someone who passes the `ops` gate submits every open question. Console on port 6533 does that as an operator: the route sends the signed-in operator and that session's auth scopes into the review gate. The gate must allow them. An operator may resolve any tenant. A user-plane caller whose tenant does not match the park is 403.

Open `/flows/decisions`, or POST the review id from the queue:

```bash
curl -X POST http://127.0.0.1:6533/console/decisions/resolve \
  -H "authorization: Bearer $OKE_OPERATOR_AUTHORIZATION" \
...
```json
{
  ...
  "$": {
    ...
    "team": { "how": "reviewed", "p": 0.42, "raw": { "billing": 0.2, "technical": 0.2, "none_of_these": 0.6 } },
...
| `suspended` | Drift: audit labels since certification, inside 7 days, for the pinned model, failed the error cap. The monitor emitted `oke/decision/drift`. | The next boot, once the lockfile's `certifiedAt` is newer than that flag. |
...
The review gate must allow the caller. Console resolve runs on the operator plane: it passes the signed-in operator id and the session auth into the gate, and it may resolve any tenant. A user-plane caller whose tenant does not match the park is 403.
...
Point `evals` at that file, then certify. The command loads the app the same way boot does, so the decision is in the registry. Prompt evals do not run when the flag is present.
It prints one JSON line per decision and writes `oke-decisions.lock.json` in the app root (`OKE_ROOT_DIR`, or `rootDir`), not in the shell's current directory. Boot reads that same file. Declared decisions without a project root fail at boot.
...
```text
{"triage":{"model":"typesafe/jev-1.13","certifiedAt":1710000000000,"questions":{"team":{"":{"hash":"…","calibrator":{"kind":"temperature","t":1.2},"threshold":0.8,"metrics":{"labels":122,"accepted":122,"errors":0,"maxError":0.05,"delta":0.1}}}}}}
```
      ...
      "questions": {
        "team": {
          "": {
            ...
            "threshold": 0.8,
            "metrics": { "labels": 122, "accepted": 122, "errors": 0, "maxError": 0.05, "delta": 0.1 }
          }
        }
      }
    }
...
After production labels exist, the clock stores a candidate on the journal. Any instance can serve it, including after a restart. Promote fetches it. It does not recompute the certificate. Other decisions already in the file stay. This decision's slot is replaced. The running process keeps the old drift flag. The next boot with this lockfile clears it when the new `certifiedAt` is newer than the flag.
...
<Accordion title="400, 401, 402, 403, 404, or 422 from the provider">
  `DecisionRequestError`. 400 and 422 are a body the provider rejected. 401 is the key. 402 is billing. 403 is forbidden. 404 is an unknown model or path. The breaker stays closed. Fix the request. Do not retry as if the provider were down.
</Accordion>

<Accordion title="oke boot: decisions are declared but rootDir and OKE_ROOT_DIR are unset">
  `oke boot: decisions are declared but rootDir and OKE_ROOT_DIR are unset, so the decision lockfile cannot load.` Boot with `rootDir`, or set `OKE_ROOT_DIR` to the directory that holds `oke-decisions.lock.json`.
</Accordion>

<Accordion title="autonomy requires audit">
  `ai.decision("triage"): autonomy requires audit`. `autonomy` without `audit` fails to compile. Set `audit` from 0 to 1. `maxError` is required in the same object.
</Accordion>

<Accordion title="options must be an object literal">
  `ai.decision("triage"): choice "team" options must be an object literal`. Write the options inline. A variable is invisible to the compiler. The same applies when the review gate name cannot be resolved: `ai.decision: review gate name could not be resolved`.
</Accordion>

<Accordion title="The candidate route is missing">
  The candidate route is registered only when some decision declares `autonomy`. Without that block there is no lockfile grant to promote, and `GET /_oke/decisions/:name/candidate` is not mounted.
</Accordion>
