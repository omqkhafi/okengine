...
Default `maxSteps` is **6**. `stopReason` is `completed`, `max_steps` (`ok: false`),
`budget` (partial trail, no throw), or `error` (a thrown tool, rethrown). A denial fed
back to the model stays `completed`.
