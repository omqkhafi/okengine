...
Default `maxSteps` is **6** when omitted. Hitting the step cap ends the loop. Spending
`budget.maxCostPerRun` stops the loop and `fx.run` returns the steps already finished.
