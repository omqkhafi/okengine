...
Named clocks still carry health — drift, overdue, missed runs + catch-up `"one"`, and the lease holder. `/monitoring` shows the same overdue / lease chips — not a live process count.
