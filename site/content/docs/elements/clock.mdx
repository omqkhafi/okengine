...
Completed journal steps never re-run on resume — a process killed mid-flow continues at the next unfinished step.
...
**Leader election needs a shared store.** The default in-memory CronStore is per process. Multi-replica exclusivity only holds when every instance opens the same durable CronStore (file-backed store is available for that; a Postgres cron driver is not wired yet).

**Catch-up example:** an hourly clock down for five hours reports `missedRuns: 5` and `catchUp: "one"`, then a single tick runs the handler once.
...
| `postgres` | Accepted in config — **not implemented yet** (boot still uses memory) |
