...
`/monitoring` adds an Instances chip from `GET /console/instances` (alive ⇔ presence TTL not expired). A cron lease holder is not a fleet census.
...
<Accordion title="Monitoring says no fleet">
