...
This one rule is what made the month-8 drift visible: every effect is in the Manifest, every run is in `fx.runs`, and a duplicate is a second run you can see. The fix stays one line — a unique constraint or upsert — not a second system.
...
| **Month 6 double-submit** — dedup split across a queue and a DB      | The duplicate is two runs in `fx.runs`. The fix is one line: a unique constraint or upsert. `{ key }` serializes `once` per key. It does not deduplicate. |
