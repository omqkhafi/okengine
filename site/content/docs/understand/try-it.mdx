...
<Accordion title="Unit mixes index.ts with other route files">
  Cause: `Unit "users" mixes a barrel index.ts with tree route files.` Keep only `index.ts`
  (and `shapes.ts`). Signals in `src/signals/`; workers in another unit (`workers/welcome.ts`).
  After deleting `index.ts`, tree files (`list.ts` / `create.ts`) can share the folder.
</Accordion>
