...
| Status  | `200` (`fx.json.create` → `201`) | `422` if `in` fails; declared `fx.fail` codes → `400` |
...
<Accordion title="Unit mixes index.ts with other route files">
  ...
  `shapes.ts`). Declare signals in `src/signals/`; bind workers in another unit
  (`workers/welcome.ts`). After you delete `index.ts`, tree files (`list.ts` / `create.ts`) can
  share the folder.
</Accordion>
...
<Accordion title="VaultBootError at startup">
  A contract has no value in any resolution layer. The error lists every gap — set the missing names
