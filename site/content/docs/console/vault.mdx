...
<Cards>
  <Card title="Fingerprints" description="Salted hash per environment — rotation and drift without exposure." />
  <Card title="Resolution chain" description="Which source won: env, files, vault driver, or fallback." />
  <Card title="Who can read" description="Derived from effects that declare `fx.vault`." />
  <Card title="Rotation blast radius" description="In-flight durable runs that wake with a new key." />
</Cards>
