...
Two directions, same protocol. **Serve:** OKE exposes your running app on port **6535** so an agent can read the Manifest and take confirmed actions. **Consume:** your Flows call *external* MCP servers as tools — declared on `ai.mcpServer`, dispatched through `fx.call`.
