...
| `oke.ai.runs.list`      | Agent runs (`limit` ≤ 200, optional `tenant`)       | `mcp:ai:read` · `console:runs:read`           |
| `oke.ai.runs.get`       | One agent run plus its follow events (`runId`)      | `mcp:ai:read` · `console:runs:read`           |
| `oke.ai.approvals.list` | Pending tool approvals (optional `tenant`)          | `mcp:ai:read` · `console:runs:read`           |
| `oke.decisions.list`    | Decision state, pending count, metrics, and drift   | `mcp:decisions:read` · `console:runs:read`    |
