...
| Durable KV         | `appendonly` / `appendfsync` are **not supported** on Redis OSS ≥2.8.22 or Valkey. Snapshots + Multi-AZ only. AWS's durable product is MemoryDB — not an oke recipe. |
