...
Postgres is the default `store.sql` image in docker and prod. `oke docker` matches any
image whose reference contains `postgres`, `pgvector`, or `timescale`, then derives the
compose service — env, healthcheck, and connection URL.

<Callout title="The one rule">
  The driver id stays `postgres` no matter which Postgres-wire image you pin — vendor
  choice lives in `images["store.sql"]`, not in `drivers.store.sql`.
</Callout>
...
images: {
  "store.sql": "postgres:18-alpine", // any Postgres / pgvector / Timescale image
},
```
