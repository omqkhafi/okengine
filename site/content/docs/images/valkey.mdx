...
<Callout title="The one rule">
  Driver id stays `redis`. Valkey is an image choice, not a driver. `REDIS_URL` and every
  KV call through `fx.store` work exactly as they do against Redis.
</Callout>
...
No named volume in the recipe — same ephemeral default as [Redis](/docs/images/redis).
