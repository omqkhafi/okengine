...
<Callout title="The one rule">
  The driver id stays `redis` for every Redis-wire image — Redis, Valkey, Dragonfly, or a
  managed provider. Vendor choice lives in `images["store.kv"]`, never in
  `drivers.store.kv`.
</Callout>
...
<Callout title="License note" type="warn">
  Redis ≥8 is dual-licensed RSALv2 / SSPLv1 / AGPLv3. Those terms restrict offering Redis
  itself as a managed service to third parties — running it for your own app is unaffected.
  Prefer [Valkey](/docs/images/valkey) when the managed-service restriction matters.
</Callout>
