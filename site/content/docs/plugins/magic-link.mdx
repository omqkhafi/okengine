...
**Consequence:** the plugin contributes the `auth-magic-link` Channel template and English
catalog body (`{{link}}`, `{{token}}`). Override copy by merging your own catalog at boot.
