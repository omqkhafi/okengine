...
<Callout title="The one rule">
  Create a Sign in with Apple key (Team ID, Key ID, `.p8` private key), seed
  the key in Vault, and register your exact callback URI — Apple validates all
  three on every exchange.
</Callout>
