...
<Callout title="The one rule">
  Keep the `email` scope in the request (it is on by default). Without it
  Discord never reports `verified: true`, and unverified emails cannot claim
  existing accounts.
</Callout>
...
| Email | taken as-is when present; `null` flows through as *no email* |
