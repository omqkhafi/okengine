...
<Callout type="info" title="Pre-account hijack defense (email)">
  App-mode email verify proves inbox ownership the same way magic-link does. An
  **unverified** parked email+password account for that address is reclaimed:
  sessions revoked, password hashes cleared, `emailVerified` set, then the OTP
  session is issued. Already-verified owners re-auth without a credential purge.
  Phone OTP is unchanged (no email identity).
</Callout>
...
<Cards>
  ...
  <Card title="Two-factor" description="TOTP / email OTP second factor." href="/docs/plugins/two-factor" />
