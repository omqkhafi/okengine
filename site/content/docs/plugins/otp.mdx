...
<Callout type="info" title="Pre-account hijack defense (email)">
  App-mode email verify proves inbox ownership. An **unverified** parked
  email+password account is reclaimed (sessions revoked, hashes cleared,
  `emailVerified` set) before the OTP session. Verified owners re-auth only.
</Callout>
