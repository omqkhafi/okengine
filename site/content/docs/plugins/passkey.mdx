...
Options return a challenge plus a ceremony `sessionId`; register and authenticate verify
`clientDataJSON` type · origin · challenge, `authenticatorData` rpId hash · UP · UV, and an
ECDSA P-256 signature against the stored SPKI public key. Challenges expire in ≤5 minutes
and are single-use.
