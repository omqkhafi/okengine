...
Several Flow exports in one `route.ts` share the inferred path; different methods are OK. The same method twice fails boot (**OKE1041**).
...
`http.get()` with no path is a sentinel, not `/`. Import the generated barrel so the stamp runs, or pass an explicit path. Boot fails with **OKE1040** if the sentinel is still there.

</Accordion>

<Accordion title="Duplicate method + path">

Two bindings on the same method and path fail boot (**OKE1041**) — last-add-wins is refused. `list.ts` (`GET /notes`) plus a `GET` in `route.ts` is that collision. `list.ts` + `create.ts` share `/notes` on different methods and are fine.

</Accordion>

<Accordion title="Stale generated.ts">

A unit folder on disk with no adopted flows is **OKE1030**. Run `oke dev` or `oke build`. The watcher regenerates on `flows/**` changes except `generated.ts` itself.
...
`flow({ do })` outside a unit is `flow_1` until stamped. Adopted HTTP still unnamed is **OKE1045**. Export from `flows/<unit>/` or pass `flow("unit.export", {…})`.
