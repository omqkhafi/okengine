...
const STRUCTURAL =
  /^(<(Cards|Card|Callout|Tabs|Tab|TypeTable|Features|Steps|Step|Accordions?)\b|```|#{1,6} |\|)/;
...
describe("docs prose density", () => {
  test(`index + elements + console MDX ≤ ${MAX_CONSECUTIVE} consecutive plain paragraphs`, async () => {
    const files = [
