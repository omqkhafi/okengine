...
export const HERO_CODE_LINES: ReadonlyArray<{
  ...
  { text: "  http.post({ in: LinkCreateIn, out: LinkOut })", beat: 0 },
  { text: "    .gate(linksMutate),", beat: 1 },
  { text: "  flow({", beat: 0 },
  { text: "    do: async (input, fx) => {", beat: null },
  { text: "      await fx.vault.get(publicApiUrl);", beat: 2 },
  { text: "      const id = fx.id();", beat: null },
  { text: "      const createdAt = fx.clock.now();", beat: 3 },
  { text: "      const summary = await fx.ask(summarize, { body: input.url });", beat: 4 },
  { text: "      await fx.store(db).insert(links).values({ … });", beat: 5 },
  { text: "      await fx.emit(linkCreated, { id, code }, { key: id });", beat: 6 },
  { text: "      await fx.send(linkCreatedMail, { to, data: { id, code } });", beat: 7 },
  { text: "      return { id, code, summary, createdAt };", beat: null },
  { text: "    },", beat: null },
  { text: "  }),", beat: null },
  { text: ");", beat: null },
];
