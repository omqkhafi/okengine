...
export function VaultResolution() {
  ...
  /* Reduced motion freezes run 2's outcome: a hit on `.env.local`. */
  const t = tick ?? BEATS_PER_RUN + 5;
  const run = Math.floor(t / BEATS_PER_RUN);
  const beat = t % BEATS_PER_RUN;
  /* Hit layer 0–4 across runs 0–4; run 5 misses everything. */
  const hitAt = run % 6 === 5 ? -1 : run % 5;
  ...
  return (
    ...
    >
      ...
      <RevealGroup as="ol" className="flex flex-col gap-px bg-fd-border">
        {LAYERS.map((layer, i) => {
          const probing = beat < 5 && i === beat && (hitAt === -1 || beat <= hitAt);
        ...
        <RevealItem
          ...
          className={cn(
            "flex min-w-0 flex-wrap items-baseline gap-x-3 gap-y-0.5 px-4 py-2.5 transition-colors duration-300 sm:px-5",
            hitAt === -1 && beat === 5 ? fail.lit : "bg-fd-secondary/50",
          )}
        >
          ...
          <span
            ...
            className={cn(
              "ml-auto font-mono text-[10px] transition-opacity duration-300",
              hitAt === -1 && beat === 5 ? fail.mark : "opacity-0",
            )}
          >
            VaultBootError
          </span>
