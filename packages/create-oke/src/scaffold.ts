...
export type ScaffoldOptions = {
   ...
   * `oke.config.ts` `store.sql` local/docker/prod. Default `sqlite`.
   */
...
export function scaffold(options: ScaffoldOptions): ScaffoldResult {
  ...
  try {
    ...
    // Post-scaffold: copy `.env.example` → `.env.local` so Vault resolves on
    // first `oke dev` without a manual step. `.env.local` stays gitignored.
 ...
 */
function applySqlDriverTransforms(
  targetDir: string,
  sqlDriver: SqlDriverId,
): void {
  const schemaPath = join(targetDir, "src/schema.ts");
  if (existsSync(schemaPath)) {
    const next = transformSchemaForSqlDriver(
  ...
  if (existsSync(configPath)) {
    const next = transformConfigForSqlDriver(
