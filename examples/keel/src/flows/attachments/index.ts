...
export const upload = on(
  http.post("/attachments", { in: UploadIn, out: IdOut }).gate(member),
...
export const get = on(
  http.get("/attachments/:id", { in: IdIn, out: FileRef }).gate(member),
...
export const remove = on(
  http.delete("/attachments/:id", { in: IdIn, out: Ok }).gate(member),
